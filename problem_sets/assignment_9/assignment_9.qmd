...
The simple character vector does not have values from other columns (variables) associated with it; however variable as factor (as in model column) has values linked to each of the observation to other variables. Similarly, converting a column as a factor suggests R to not stretch the analysis beyond those values (or use only the given values) and do not assume they fall in any scale or follow any ordered (unless otherwise directed). Likewise, reordering the levels makes the plot less messy (otherwise the observations are scattered and difficult to read and they become more messy to label).
...
I used chatgpt to check the issue with the code for reordering the levels. The way I had written the code was suggesting to create subset rows, which I was not trying do and chatgpt flagged that, helping me to revise the code.
